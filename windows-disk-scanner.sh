...
# Username, password & API-endpoint
...
hosts=$(curl -s -g -k -X GET -u "$username:$password" "https://$op5_host/api/filter/query?query=[hosts]%20groups%20%3E=%20%22Windows%22&columns=name")
if [ $? -ne "0" ]
then
